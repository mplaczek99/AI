...
def bidirectional_iddfs(graph, start, goal):
    ...
    while True:
        ...
        found_from_start, count_from_start = dls(
            graph,
            start,
            goal,
            depth,
        ...
        found_from_goal, count_from_goal = dls(
            graph,
            goal,
            start,
            depth,
        ...
        total_call_count += (
            count_from_start + count_from_goal
        )  # Aggregate the count of calls
        ...
        if found_from_start and found_from_goal:
            return (
                [],
                total_call_count,
            )  # One of the DLS found the goal, this shouldn't happen in a correct setup
...
def dls(graph, node, goal, depth, visited, path, call_count, forward):
    ...
    for neighbor in graph[node]:
        if neighbor not in visited:
            found, call_count = dls(
                graph, neighbor, goal, depth - 1, visited, path, call_count, forward
            )
            if found:
                return True, call_count
